...
export const Route = createFileRoute("/_authenticated/ordens/")({
  head: () => ({ meta: [{ title: "Ordens de Serviço — Manutenção Xica da Silva" }] }),
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{filtered.length} de {ordens.length}</p>
        </div>
      ...
      <Card>
        <CardContent className="p-0">
          <div className="divide-y">
            {filtered.length === 0 && (
              <div className="p-8 text-center text-sm text-muted-foreground">Nenhuma OS encontrada.</div>
            )}
            {filtered.map((o) => {
